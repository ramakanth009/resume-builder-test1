...
# app.config['SQLALCHEMY_DATABASE_URI'] = f"postgresql://{os.getenv('DB_USER')}:{quote_plus(os.getenv('DB_PASSWORD'))}@{os.getenv('DB_HOST')}:{os.getenv('DB_PORT')}/{os.getenv('DB_NAME')}"
...
#     required_fields = {'name', 'skills_used', 'description'}
#     return (
...
def normalize_resume_structure(ai_output):
    ...
    try:
        ...
        normalized['projects'] = [{
            ...
            #     proj.get('skills_used', '').split(', ') if proj.get('skills_used') else []
            # )
...
#     required_fields = ['name', 'template_code']
...
# @app.route('/api/render_resume', methods=['POST'])
# def render_resume():
#     """Render a resume with a specific template."""
#     data = request.get_json()
    
#     if not data or 'template_id' not in data or 'resume_data' not in data:
#         return jsonify({
#             'status': 'error',
#             'message': 'Missing template_id or resume_data'
#         }), 400
    
#     template = ResumeTemplate.query.get_or_404(data['template_id'])
...
#     """Serve thumbnail images from static/thumbnails directory."""
